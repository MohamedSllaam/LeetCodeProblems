...
SELECT mx.store_id ,s.store_name,s.location ,mx.product_name most_exp_product ,
mi.product_name cheapest_product , 
CAST(ROUND(1.0 * mi.quantity / mx.quantity, 2) AS DECIMAL(10,2)) imbalance_ratio
...
join stores s
 on mx.store_id = s.store_id
 join  inventory iin
 on mx.store_id = iin.store_id
 where mx.quantity < mi.quantity
 group by 
 mx.store_id ,s.store_name,s.location ,mx.product_name  ,
mi.product_name  , 
CAST(ROUND(1.0 * mi.quantity / mx.quantity, 2) AS DECIMAL(10,2)) 
  having count( iin.store_id) >=3
 order by imbalance_ratio desc , store_name asc
